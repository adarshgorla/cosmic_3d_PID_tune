// =============================================================================
// Cosmic3D MK9 - Upward Homing Rotation & Speed Experiment
// ESP32-S3 | 12V DC Motors + L293D H-Bridge | AS5600 Encoders | TCA9548A I2C Mux
// =============================================================================
// Objective of Experiment:
// 1. User manually sets the central pole / effector to origin (0,0,0).
// 2. Encoders zeroed at origin.
// 3. Motors move upward simultaneously until their respective limit switches click.
// 4. Measures, logs, and displays:
//    - Total motor rotations for each tower (A, B, C)
//    - Instantaneous and average rotational speed (RPM)
//    - Linear cable speed (mm/s)
//    - Time elapsed per motor and final summary report
// 5. Stores logs to SD Card (/homing_experiment_log.csv) and displays on Serial.
// =============================================================================
...
#define UPWARD_DRIVE_PWM 180       // Motor drive PWM for upward movement (0 - 255)

// Limit Switches (INPUT_PULLUP: HIGH = unclicked, LOW = clicked)
#define SWITCH_CLICKED LOW

// =============================================================================
// GLOBAL STATE & ENCODER VARIABLES
// =============================================================================
...
void initEncodersAndZeroAtOrigin();
void initSDLog();
...
void updateEncoderFromAS5600(int motorIndex) {
  ...
  // Instantaneous RPM Calculation
  unsigned long now = millis();
  float dtSec = (now - prevSpeedCalcTime[motorIndex]) / 1000.0f;
  if (dtSec >= 0.05f) { // Update speed at least every 50ms
...
void initEncodersAndZeroAtOrigin() {
  ...
  Serial.println("[EXP] Initializing I2C & AS5600 Encoders...");

  as5600Ready = true;

  for (int i = 0; i < 3; i++) {
    if (!selectI2CChannel(MOTOR_TO_I2C_CHANNEL[i])) {
      Serial.printf("[EXP] ❌ Motor %d (Channel %d) I2C Select Failed!\n", i, MOTOR_TO_I2C_CHANNEL[i]);
      as5600Ready = false;
      continue;
    }
    delay(5);
    ...
    Serial.printf("[EXP] Motor %d (Ch %d): Initial Angle = %u | Magnet OK = %s\n",
                  i, MOTOR_TO_I2C_CHANNEL[i], angle, magOK ? "YES" : "NO");
  ...
  if (as5600Ready) {
    Serial.println("[EXP] ✅ Encoders zeroed at origin successfully.");
  } else {
    Serial.println("[EXP] ⚠️ Encoder initialization had warnings/failures!");
  }
}
...
void initSDLog() {
  ...
  if (!SD.begin(SD_PIN_CS, sdSPI)) {
    Serial.println("[SD LOG] ❌ SD Card initialization failed! Telemetry will be logged to Serial only.");
    sdReady = false;
    return;
  }
  sdReady = true;
...
void logTelemetryToSDAndSerial() {
  ...
  bool swA = digitalRead(LIMIT1_PIN) == SWITCH_CLICKED;
  bool swB = digitalRead(LIMIT2_PIN) == SWITCH_CLICKED;
  bool swC = digitalRead(LIMIT3_PIN) == SWITCH_CLICKED;

  // Print formatted live status to Serial Monitor
...
void startExperiment() {
  ...
  Serial.println("[EXP] Make sure pole is set at origin manually.");
  Serial.println("[EXP] Zeroing encoders and starting upward motion...");
...
void loop() {
  ...
  case EXP_STATE_RUNNING: {
    ...
    // 2. Check Limit Switches for each tower
    if (!motorHitLimit[0] && digitalRead(LIMIT1_PIN) == SWITCH_CLICKED) {
      stopTowerMotor(0);
    ...
    if (!motorHitLimit[1] && digitalRead(LIMIT2_PIN) == SWITCH_CLICKED) {
      stopTowerMotor(1);
    ...
    if (!motorHitLimit[2] && digitalRead(LIMIT3_PIN) == SWITCH_CLICKED) {
      stopTowerMotor(2);

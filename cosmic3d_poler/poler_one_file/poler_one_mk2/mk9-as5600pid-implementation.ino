...
const float R_BED_DIAMETER_MM = 121.5f;
...
const float Z_COUNTS_PER_MM = 68.0f;
const float INTERPOLATION_SEGMENT_MM = 1.0f;
...
/*
 * Micro SD Card SPI Pinout (ESP32-S3):
 * ------------------------------------
 * pin 1 - not used / DAT2   |  Micro SD card     |
 * pin 2 - CS (SS)   -> GPIO 10                   /
 * pin 3 - DI (MOSI) -> GPIO 11                  |__
 * pin 4 - VDD (3.3V)                            |
 * pin 5 - SCK (SCLK)-> GPIO 12  | 8 7 6 5 4 3 2 1   /
 * pin 6 - VSS (GND)             | ▄ ▄ ▄ ▄ ▄ ▄ ▄ ▄  /
 * pin 7 - DO (MISO) -> GPIO 13  | ▀ ▀ █ ▀ █ ▀ ▀ ▀ |
 * pin 8 - not used / DAT1       |_________________|
 *
 * Pin assignments on ESP32-S3:
 * CS   = GPIO 10
 * MOSI = GPIO 11
 * SCK  = GPIO 12
 * MISO = GPIO 13
...
#define motorPinA1 15 // R Motor Pin 1
#define motorPinA2 16 // R Motor Pin 2
#define motorPinB1 14 // Z Motor Pin 1
#define motorPinB2 7  // Z Motor Pin 2
...
const uint8_t motorToChannel[4] = {1, 0, 2, 3};
...
long setpointHome[4] = {0, -680, 0,
                        0}; // Index 1: Z Standoff (-680 counts = 10 mm UP)
...
// (Defined in cosmic3d-led-test-esp-32_S3.ino to avoid forward declaration
// errors during Arduino compilation)
...
bool isPrinterActive() { return isActivePrintState(sysState); }
...
long zMmToEncoderCounts(float zMm) {
  // Inverted: Z+ (up) is counts minus, and counts minus is Z+
...
// =============================================================================
// SD CARD HELPERS & INITIALIZATION
// =============================================================================
...
void testFileIO(fs::FS &fs, const char *path) {
  ...
  if (file) {
    ...
    Serial.printf("[SD] %lu bytes read for %" PRIu32 " ms\n",
                  (unsigned long)flen, end);
    file.close();
  } else {
    Serial.println("[SD] Failed to open file for reading test");
  }

  file = fs.open(path, FILE_WRITE);
  if (!file) {
    Serial.println("[SD] Failed to open file for writing test");
    return;
  }
  ...
  Serial.printf("[SD] %u bytes written for %" PRIu32 " ms\n", 2048 * 512, end);
  file.close();
}
...
void stopAllMotors() {
  ...
  // Use digitalWrite for auxiliary pins to avoid exhausting ESP32-S3 LEDC
  // channels
...
void mk9Loop() {
  ...
  case STATE_HOMING_SEEK: {
    // R Bed Rotation Homing (Bypassed: no physical switch, current position set
    // as 0 deg)
    if (!r_home) {
      r_home = true;
      encoderCount[0] = 0;
      ...
      Serial.println("[HOME] R Bed reference sensor bypassed (current position "
                     "set as 0 deg).");
    }

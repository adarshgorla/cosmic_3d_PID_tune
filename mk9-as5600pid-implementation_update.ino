...
const int encoderSign[4] = {-1, 1, 1, 1}; // Tower A encoder direction is inverted
...
// Limit switch debounce: requires 3 consecutive LOW reads to confirm a click
...
void restoreState() {
  ...
  Serial.print("[NVS]   fileName: ");    Serial.println(fileName);
  Serial.print("[NVS]   totalChunks: "); Serial.println(totalChunks);
  Serial.print("[NVS]   lastChunk: ");   Serial.println(lastReceivedChunk);
}
...
void listDir(fs::FS &fs, const char *dirname, uint8_t levels) {
  ...
  if (!root || !root.isDirectory()) { Serial.println("[SD] Failed."); return; }
...
void writeFile(fs::FS &fs, const char *path, const char *message) {
  ...
  if (!f) { Serial.println("[SD] Write open failed."); return; }
  f.print(message) ? Serial.println("[SD] Written.") : Serial.println("[SD] Write failed.");
  f.close();
}

void appendFile(fs::FS &fs, const char *path, const char *message) {
  File f = fs.open(path, FILE_APPEND);  
  if (!f) { Serial.println("[SD] Append open failed."); return; }
  f.print(message) ? Serial.println("[SD] Appended ✅") : Serial.println("[SD] Append failed.");
  f.close();
}

int countLinesInFile(fs::FS &fs, const char *path) {
  File tempFile = fs.open(path);
  if (!tempFile) { Serial.println("[SD] Line count open failed."); return -1; }
  int lineCount = 0;
    ...
    float &cableC)
{
    ...
    float bx =
        centerX +
        EFFECTOR_RADIUS *
        (COS120*u1x + SIN120*u2x);

    float by =
        centerY +
        EFFECTOR_RADIUS *
        (COS120*u1y + SIN120*u2y);

    float bz =
        centerZ +
        EFFECTOR_RADIUS *
        (COS120*u1z + SIN120*u2z);
...
void updateEncoderFromAS5600(int motorIndex) {
  ...
  long diff = ((long)currentAngle - (long)lastAngle[motorIndex]) * encoderSign[motorIndex];
  ...
  if (motorCounts != 0) {
    ...
    Serial.printf("[ENC] Motor %d | Ch %d | RawAngle: %u | RawDiff: %ld | "
                  ...
                  motorIndex, motorToChannel[motorIndex],
                  currentAngle, diff,
                  rawAccumulator[motorIndex], motorCounts,
                  prevCount, encoderCount[motorIndex],
                  totalRotations[motorIndex], setpoint[motorIndex]);
  }
...
void initAS5600() {
  ...
  disableAllI2CChannels();
  Serial.println(as5600Ready ? "[AS5600] Init complete." : "[AS5600] Init degraded (offline channels).");
}
...
void checkAllEncoders() {
  ...
  for (int i = 0; i < 4; i++) {
    ...
    Serial.printf("[DIAG] Motor %d | Angle=%u | Count=%ld | RawAcc=%ld | "
                  ...
                  i, angle, encoderCount[i], rawAccumulator[i],
                  setpoint[i], magnitude, agc, status,
                  magOK ? "✓ OK" : "✗ MISSING!");
    delay(1);
  }
  disableAllI2CChannels();
...
void runHeaterControl() {
  ...
  Serial.printf("[HEAT] Hotend: %.1f°C [%s / sp %.1f°C] | Bed: %.1f°C [%s / sp %.1f°C]\n",
                hotendTemp, hotendHeaterOn ? "ON" : "OFF", hotendSetpoint,
                bedTemp,    bedHeaterOn    ? "ON" : "OFF", bedSetpoint);
}
...
void driveMotor(int i, float output) {
  ...
  if (i == 0) {
    if (output > 0)      { analogWrite(motorPinA1, 0);   analogWrite(motorPinA2, pwm); }
    else if (output < 0) { analogWrite(motorPinA1, pwm); analogWrite(motorPinA2, 0);   }
    else                 { analogWrite(motorPinA1, 0);   analogWrite(motorPinA2, 0);   }
  } else if (i == 1) {
    if (output > 0)      { analogWrite(motorPinB1, 0);   analogWrite(motorPinB2, pwm); }
    else if (output < 0) { analogWrite(motorPinB1, pwm); analogWrite(motorPinB2, 0);   }
    else                 { analogWrite(motorPinB1, 0);   analogWrite(motorPinB2, 0);   }
  } else if (i == 2) {
    if (output > 0)      { analogWrite(motorPinc1, 0);   analogWrite(motorPinc2, pwm); }
    else if (output < 0) { analogWrite(motorPinc1, pwm); analogWrite(motorPinc2, 0);   }
    else                 { analogWrite(motorPinc1, 0);   analogWrite(motorPinc2, 0);   }
  } else {
    if (output > 0)      { analogWrite(motorPind1, 0);   analogWrite(motorPind2, pwm); }
    else if (output < 0) { analogWrite(motorPind1, pwm); analogWrite(motorPind2, 0);   }
    else                 { analogWrite(motorPind1, 0);   analogWrite(motorPind2, 0);   }
  }
...
void mk9Setup() {
  ...
  pinMode(motorPinA1, OUTPUT); pinMode(motorPinA2, OUTPUT);
  pinMode(motorPinB1, OUTPUT); pinMode(motorPinB2, OUTPUT);
  pinMode(motorPinc1, OUTPUT); pinMode(motorPinc2, OUTPUT);
  pinMode(motorPind1, OUTPUT); pinMode(motorPind2, OUTPUT);
  stopAllMotors();
  delay(1);
  ...
  if (!as5600Ready) {
    Serial.println("[SETUP] AS5600/TCA9548A offline. Entering safe mode (WiFi active, motion disabled).");
    return;
  }
  ...
  mqttClient.onMessage([](int messageSize) {
    ...
    Serial.printf("[MQTT] ← %s  (%d bytes)\n", topicBuffer.c_str(), (int)payloadBuffer.length());
  });

  if (WiFi.status() == WL_CONNECTED) {
    Serial.print("[MQTT] Connecting to "); Serial.println(MQTT_BROKER);
    if (!mqttClient.connect(MQTT_BROKER, 1883)) {
      Serial.print("[MQTT] Failed, error="); Serial.println(mqttClient.connectError());
    } else {
  ...
  // Skip immediate full diagnostics at startup to avoid long I2C burst right after WiFi attach.
  mk9CoreReady = true;
}
...
void mk9Loop() {
    ...
    if (topicBuffer == MOTOR_STOP ) {
      if (payloadBuffer == "STOP") {
        ...
        if (isActivePrintState(sysState)) {
          ...
          Serial.println("[MOTION] Pause requested but no active print running.");
        }
      }
    }
    ...
    // -------------------------------------------------------------------------
      ...
      if (payloadBuffer.indexOf('|') != -1) {
        ...
        if (command == "home" && sysState == STATE_IDLE) {
          ...
          Serial.println("[CMD] End of file transfer command received, but no file transfer in progress.");
          MD5Builder md5;
          ...
          if (f) {
            ...
            if (computed.equalsIgnoreCase(expectedChecksum)) {
              ...
              mqttClient.beginMessage(TOPIC_ACK); mqttClient.print("SUCCESS"); mqttClient.endMessage();
            } else {
              Serial.println("[XFER] ❌ Checksum MISMATCH.");
              mqttClient.beginMessage(TOPIC_ACK); mqttClient.print("FAIL_CHECKSUM"); mqttClient.endMessage();
            }
          } else {
      ...
      if (firstChar == 'X' || firstChar == 'Y' || firstChar == 'Z') {
        if (cmd.length() >= 2) {
          ...
          if (sign == '+' || sign == '-') {
            ...
            if (axis == 'X') dx = amount;
            else if (axis == 'Y') dy = amount;
            else if (axis == 'Z') dz = amount;
            valid = true;
          }
        }
      } else {
        // Legacy/alternate format support (e.g. x+, y-, z+)
        if (cmd == "X+") { dx = MANUAL_JOG_MM; valid = true; }
        else if (cmd == "X-") { dx = -MANUAL_JOG_MM; valid = true; }
        else if (cmd == "Y+") { dy = MANUAL_JOG_MM; valid = true; }
        else if (cmd == "Y-") { dy = -MANUAL_JOG_MM; valid = true; }
        else if (cmd == "Z+") { dz = MANUAL_JOG_MM; valid = true; }
        else if (cmd == "Z-") { dz = -MANUAL_JOG_MM; valid = true; }
      }

      if (valid) {
        Serial.printf("[CMD] Received XYZ move command: %s  → delta (dx:%.2f dy:%.2f dz:%.2f)\n",
                      cmd.c_str(), dx, dy, dz);
        if (isActivePrintState(sysState)) {
          Serial.println("[CMD] Manual XYZ move requested during active print. Pausing print first.");
    ...
    // -------------------------------------------------------------------------
      ...
      if (payloadBuffer == "RESUME" && (sysState == STATE_MOTION_PAUSED || sysState == STATE_MANUAL_XYZ)) {
        if (pausedPrintState != STATE_IDLE) {
          ...
          Serial.println("[MOTION] Resume requested but no paused print available. Returning to IDLE.");
          stopAllMotors();
          sysState = STATE_IDLE;
        }
  ...
  switch (sysState) {
    ...
    // --- HOMING: drive toward limit switches (debounced) ---
    case STATE_HOMING_SEEK: {
      if (!m1_home) {
        analogWrite(motorPinA1, 0); analogWrite(motorPinA2, 255);
        if (checkDebouncedLimitSwitch(0, LIMIT1_PIN)) {
          analogWrite(motorPinA2,0);
          m1_home = true;
          encoderCount[0] = 0; rawAccumulator[0] = 0;
          Serial.println("[HOME] Axis 0 limit reached (debounced).");
        }
      }
      if (!m2_home) {
        analogWrite(motorPinB1, 0); analogWrite(motorPinB2, 255);
        if (checkDebouncedLimitSwitch(1, LIMIT2_PIN)) {
          analogWrite(motorPinB2, 0);
          m2_home = true;
          encoderCount[1] = 0; rawAccumulator[1] = 0;
          Serial.println("[HOME] Axis 1 limit reached (debounced).");
        }
      }
      if (!m3_home) {
        analogWrite(motorPinc1, 0); analogWrite(motorPinc2, 255);
        if (checkDebouncedLimitSwitch(2, LIMIT3_PIN)) {
          analogWrite(motorPinc2, 0);
          m3_home = true;
          encoderCount[2] = 0; rawAccumulator[2] = 0;
          Serial.println("[HOME] Axis 2 limit reached (debounced).");
        }
      }
    ...
    case STATE_HOMING_STANDOFF: {
      ...
      bool allHome = true;
      for (int i = 0; i < 4; i++) {
        if (abs(setpointHome[i] - encoderCount[i]) > MOTION_TOLERANCE_COUNTS) allHome = false;
      }
    ...
    case STATE_MOTION_READ_LINE: {
      ...
      if (!parseCartesianLine(motionLine, targetX, targetY, targetZ, targetE)) {
        Serial.println("[MOTION] Invalid line format, expected X Y Z E floats. Skipping.");
        myconut++;
        dwellStart = millis();
        sysState = STATE_MOTION_DWELL;
        break;
      ...
      Serial.printf("[MOTION] XYZE: %.3f %.3f %.3f %.3f\n",
                    targetX, targetY, targetZ, targetE);

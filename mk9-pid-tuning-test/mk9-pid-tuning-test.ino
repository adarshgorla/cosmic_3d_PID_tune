...
float Kp[MOTOR_COUNT] = {DEFAULT_KP[0], DEFAULT_KP[1], DEFAULT_KP[2], DEFAULT_KP[3]};
float Ki[MOTOR_COUNT] = {DEFAULT_KI[0], DEFAULT_KI[1], DEFAULT_KI[2], DEFAULT_KI[3]};
float Kd[MOTOR_COUNT] = {DEFAULT_KD[0], DEFAULT_KD[1], DEFAULT_KD[2], DEFAULT_KD[3]};
...
void startTestStep(TestState newState, float targetX, float targetY, float targetZ);
...
void initSDLog() {
  ...
  if (!SD.begin(SD_PIN_CS, sdSPI)) {
    Serial.println("[SD LOG] ❌ SD Card initialization failed! Telemetry will be output to Serial only.");
    sdReady = false;
    return;
  }
  sdReady = true;
  ...
  if (logFile) {
    if (logFile.size() == 0) {
      logFile.println("TimestampMs,State,TargetX,TargetY,TargetZ,SetpointA,SetpointB,SetpointC,CountA,CountB,CountC,ErrA,ErrB,ErrC,Status");
    }
...
void logTelemetry(const char* statusMessage) {
  ...
  Serial.printf("[%lu ms] State:%d | TgtXYZ:(%.1f,%.1f,%.1f) | Setpoints:(%ld,%ld,%ld) | Counts:(%ld,%ld,%ld) | Errs:(%ld,%ld,%ld) | %s\n",
                now, (int)testState, currentX, currentY, currentZ,
                setpoint[0], setpoint[1], setpoint[2],
                encoderCount[0], encoderCount[1], encoderCount[2],
                errA, errB, errC, statusMessage);

  // Write to SD Log file
  if (sdReady) {
    File logFile = SD.open(LOG_FILE_PATH, FILE_APPEND);
    if (logFile) {
      logFile.printf("%lu,%d,%.2f,%.2f,%.2f,%ld,%ld,%ld,%ld,%ld,%ld,%ld,%ld,%ld,%s\n",
                     now, (int)testState, currentX, currentY, currentZ,
                     setpoint[0], setpoint[1], setpoint[2],
                     encoderCount[0], encoderCount[1], encoderCount[2],
                     errA, errB, errC, statusMessage);
...
uint16_t readAS5600Register16(uint8_t reg) {
  ...
  if (Wire.requestFrom((uint8_t)AS5600_ADDRESS, (uint8_t)2, (uint8_t)true) >= 2) {
...
uint16_t readRawAngle() {
  return readAS5600Register16(AS5600_RAW_ANGLE_H);
}
...
bool calculateIK(float nozzleX, float nozzleY, float nozzleZ, float &cableA, float &cableB, float &cableC) {
  ...
  float ax = centerX + (TOWER_A_X / sqrtf(TOWER_A_X*TOWER_A_X + TOWER_A_Y*TOWER_A_Y)) * EFFECTOR_RADIUS;
  float ay = centerY + (TOWER_A_Y / sqrtf(TOWER_A_X*TOWER_A_X + TOWER_A_Y*TOWER_A_Y)) * EFFECTOR_RADIUS;
  float az = centerZ;

  float bx = centerX + (TOWER_B_X / sqrtf(TOWER_B_X*TOWER_B_X + TOWER_B_Y*TOWER_B_Y)) * EFFECTOR_RADIUS;
  float by = centerY + (TOWER_B_Y / sqrtf(TOWER_B_X*TOWER_B_X + TOWER_B_Y*TOWER_B_Y)) * EFFECTOR_RADIUS;
  float bz = centerZ;

  float cx = centerX + (TOWER_C_X / sqrtf(TOWER_C_X*TOWER_C_X + TOWER_C_Y*TOWER_C_Y)) * EFFECTOR_RADIUS;
  float cy = centerY + (TOWER_C_Y / sqrtf(TOWER_C_X*TOWER_C_X + TOWER_C_Y*TOWER_C_Y)) * EFFECTOR_RADIUS;
  float cz = centerZ;
...
bool convertXYZToEncoderTargets(float targetX, float targetY, float targetZ, long &targetA, long &targetB, long &targetC) {
  if (!homeCableLengthReady) {
    ...
    if (!calculateIK(HOME_X, HOME_Y, HOME_Z, hA, hB, hC)) return false;
  ...
  if (!calculateIK(targetX, targetY, targetZ, cA, cB, cC)) return false;
...
void driveMotor(int i, float output) {
  ...
  if (i == 0) {
    if (output > 0)      { analogWrite(MOTOR_0_PIN_1, 0); analogWrite(MOTOR_0_PIN_2, pwm); }
    else if (output < 0) { analogWrite(MOTOR_0_PIN_1, pwm); analogWrite(MOTOR_0_PIN_2, 0); }
    else                 { analogWrite(MOTOR_0_PIN_1, 0); analogWrite(MOTOR_0_PIN_2, 0); }
  } else if (i == 1) {
    if (output > 0)      { analogWrite(MOTOR_1_PIN_1, 0); analogWrite(MOTOR_1_PIN_2, pwm); }
    else if (output < 0) { analogWrite(MOTOR_1_PIN_1, pwm); analogWrite(MOTOR_1_PIN_2, 0); }
    else                 { analogWrite(MOTOR_1_PIN_1, 0); analogWrite(MOTOR_1_PIN_2, 0); }
  } else if (i == 2) {
    if (output > 0)      { analogWrite(MOTOR_2_PIN_1, 0); analogWrite(MOTOR_2_PIN_2, pwm); }
    else if (output < 0) { analogWrite(MOTOR_2_PIN_1, pwm); analogWrite(MOTOR_2_PIN_2, 0); }
    else                 { analogWrite(MOTOR_2_PIN_1, 0); analogWrite(MOTOR_2_PIN_2, 0); }
  } else if (i == 3) {
    if (output > 0)      { analogWrite(MOTOR_3_PIN_1, 0); analogWrite(MOTOR_3_PIN_2, pwm); }
    else if (output < 0) { analogWrite(MOTOR_3_PIN_1, pwm); analogWrite(MOTOR_3_PIN_2, 0); }
    else                 { analogWrite(MOTOR_3_PIN_1, 0); analogWrite(MOTOR_3_PIN_2, 0); }
  }
}

void stopAllMotors() {
  analogWrite(MOTOR_0_PIN_1, 0); analogWrite(MOTOR_0_PIN_2, 0);
...
void handleSerialCommands() {
  ...
  if (input == "1" || input == "x") {
    ...
    startTestStep(STATE_TEST_XY_OUT, HOME_X + TEST_MOVE_DIST_MM, HOME_Y + TEST_MOVE_DIST_MM, HOME_Z);
  } else if (input == "5" || input == "yz") {
    Serial.println("\n[TEST ROUTINE] Starting Test 5: Y-Z Combined (+20mm)");
    nextStateAfterDwell = STATE_TEST_YZ_RETURN;
    startTestStep(STATE_TEST_YZ_OUT, HOME_X, HOME_Y + TEST_MOVE_DIST_MM, HOME_Z + TEST_MOVE_DIST_MM);
  } else if (input == "6" || input == "xz") {
    Serial.println("\n[TEST ROUTINE] Starting Test 6: X-Z Combined (+20mm)");
    nextStateAfterDwell = STATE_TEST_XZ_RETURN;
    startTestStep(STATE_TEST_XZ_OUT, HOME_X + TEST_MOVE_DIST_MM, HOME_Y, HOME_Z + TEST_MOVE_DIST_MM);
  } else if (input == "7" || input == "xyz") {
    Serial.println("\n[TEST ROUTINE] Starting Test 7: X-Y-Z Combined (+20mm)");
    nextStateAfterDwell = STATE_TEST_XYZ_RETURN;
    startTestStep(STATE_TEST_XYZ_OUT, HOME_X + TEST_MOVE_DIST_MM, HOME_Y + TEST_MOVE_DIST_MM, HOME_Z + TEST_MOVE_DIST_MM);
  } else if (input == "stop") {
    stopAllMotors();
    testState = STATE_IDLE;
    Serial.println("[COMMAND] ⏹️ EMERGENCY STOP EXECUTED!");
    logTelemetry("EMERGENCY_STOP");
  } else if (input.startsWith("p") || input.startsWith("i") || input.startsWith("d")) {
    ...
    if (eqPos > 0 && motorIdx >= 0 && motorIdx < MOTOR_COUNT) {
      ...
      if (gainType == 'p') Kp[motorIdx] = val;
      if (gainType == 'i') Ki[motorIdx] = val;
      if (gainType == 'd') Kd[motorIdx] = val;
...
void setup() {
  ...
  pinMode(MOTOR_0_PIN_1, OUTPUT); pinMode(MOTOR_0_PIN_2, OUTPUT);
  pinMode(MOTOR_1_PIN_1, OUTPUT); pinMode(MOTOR_1_PIN_2, OUTPUT);
  pinMode(MOTOR_2_PIN_1, OUTPUT); pinMode(MOTOR_2_PIN_2, OUTPUT);
  pinMode(MOTOR_3_PIN_1, OUTPUT); pinMode(MOTOR_3_PIN_2, OUTPUT);
  stopAllMotors();

  initSDLog();
  initAS5600();

  Serial.println("\nSystem Ready. Type '1' for X-test, '2' for Y-test, etc. Send any key for menu.");
}
...
void loop() {
  ...
  switch (testState) {
    ...
    case STATE_HOLD_DWELL: {
      // Hold position at target for 1.5 seconds before returning to origin
      stopAllMotors();
      if (millis() - dwellStartTime >= 1500) {
        Serial.println("[TEST ROUTINE] Dwell finished. Returning to Home origin (0,0,0)...");
    ...
    case STATE_TEST_XYZ_RETURN: {
      ...
      if (targetReached) {
        ...
        Serial.println("[TEST ROUTINE] ✅ Test complete! Returned safely to origin.");
        testState = STATE_IDLE;
      } else {
        for (int i = 0; i < 3; i++) {
          driveMotor(i, calculatePID(i, setpoint[i], dt));
